    ...
    if (node) {
      const updateDimensions = () => {
        if (containerRef.current) {
          ...
          // Try to get dimensions from computed style as fallback
          const computedStyle = window.getComputedStyle(containerRef.current)
          const parentWidth = containerRef.current.parentElement?.getBoundingClientRect().width || width
          const parentHeight = containerRef.current.parentElement?.getBoundingClientRect().height || height
          
          // Use measured dimensions if available, otherwise try parent or computed
          const finalWidth = width > 0 ? width : (parseFloat(computedStyle.width) || parentWidth || 400)
          const finalHeight = height > 0 ? height : (parseFloat(computedStyle.height) || parentHeight || 300)
      ...
      // Also try after delays to catch layout changes
      setTimeout(updateDimensions, 10)
      setTimeout(updateDimensions, 50)
      requestAnimationFrame(updateDimensions)
      requestAnimationFrame(() => requestAnimationFrame(updateDimensions))
    } else {
